...
const Footer = () => {
  ...
  const handleWhatsAppClick = () => {
    window.open('https://wa.me/919629298337', '_blank');
  };
  ...
  return (
    <footer className="fixed bottom-0 left-0 right-0 z-30 bg-white border-t border-black">
      <div className="flex flex-col md:flex-row justify-between items-center px-4 md:px-12 py-4 md:py-6 gap-3 md:gap-0">
        ...
        {/* Desktop layout */}
        {/* Instagram - left side on desktop */}
        <div className="hidden md:flex justify-start w-full md:w-[30%]">
          <button 
            onClick={handleInstagramClick}
            className="text-sm md:text-xl font-medium tracking-wider hover:text-gray-600 transition-colors flex items-center gap-2 cursor-pointer text-gray-800 ml-12"
            aria-label="Visit Instagram"
          >
            <Instagram className="w-5 h-5" />
            INSTAGRAM
        ...
        {/* KOODU ARCHITECTURE - center on desktop */}
        <div className="hidden md:flex justify-center w-full md:w-[40%]">
          <button
            onClick={() => navigate('/')}
            className={`text-xl font-medium tracking-wider hover:text-gray-600 transition-colors cursor-pointer ${
              location.pathname === '/' ? 'font-bold' : 'text-gray-800'
            }`}
            style={location.pathname === '/' ? { color: '#58351c' } : {}}
          >
        ...
        {/* Contact button and WhatsApp - right side on desktop */}
        <div className="hidden md:flex justify-end items-center w-full md:w-[30%]">
          <button
          ...
          <button 
            onClick={handleWhatsAppClick}
            className="text-black hover:text-gray-600 transition-colors mr-[10%] ml-6" 
            aria-label="WhatsApp"
          >
