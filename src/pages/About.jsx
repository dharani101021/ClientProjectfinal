...
const About = () => {
  ...
  // Adjust body height for horizontal scroll
  useEffect(() => {
    const updateHeight = () => {
  ...
  // Helper: Wrap sections for both layouts
  const Sections = () => (
    <>
      {/* Section 1: Hero */}
                ...
                {aboutData.story.content.map((paragraph, index) => (
                  <p
                    ...
                    className={`leading-relaxed text-base md:text-lg ${
                      index === aboutData.story.content.length - 1
                        ? 'text-gray-600'
                        : 'text-gray-700'
                    }`}
      ...
      {isDesktop ? (
        ...
        >
          <motion.div
            ref={containerRef}
            style={{ x }}
            className="flex h-full w-[500vw] overflow-hidden gap-0"
          >
            <Sections />
          </motion.div>
        </div>
