...
import { projects as project04Data } from '../data/ProjectData4.js'; // Import Project04 data
...
const Project03 = ({ isVisible = true }) => {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      const totalItems = projects.length + 2; // projects + NEXT + project04 preview
    ...
    <section className={`fixed inset-0 bg-white transition-transform duration-1000 ${isVisible ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <div className="absolute inset-0 pt-12">
        ...
        >
          ...
          {/* First image from Project04 - appears after NEXT text */}
          {project04Data.length > 0 && (
            <div
              key={`project04-preview-${project04Data[0].id}`}
              className={`
            ...
            >
              <div className={`h-full relative overflow-hidden rounded-none md:rounded-none transition-all duration-700 ${getImageOpacity(projects.length + 1)}`}>
                <img
                  src={project04Data[0].image}
                  alt={project04Data[0].title}
                  className="w-full h-full object-cover scale-100 transition-transform duration-700 group-hover:scale-105"
                />
              </div>
            </div>
